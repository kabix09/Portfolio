...
import { smallProjectsData, getSmallProjectCategories } from "@/constants/projects";
import { AppContext } from "@/context/AppContext";

export const SmallProjects = () => {
  const t = useTranslations("sections.small_projects");
  const context = useContext(AppContext);
  
  const activeProfile = context?.activeProfile || "backend";
  const [activeCategory, setActiveCategory] = useState("All");

  const categories = getSmallProjectCategories(activeProfile);

  useEffect(() => {
    setActiveCategory("All");
  }, [activeProfile]);

  const currentProfileProjects = smallProjectsData[activeProfile as keyof typeof smallProjectsData] || [];
