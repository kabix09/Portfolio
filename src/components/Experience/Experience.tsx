...
export const Experience = () => {
  ...
  const jobs = experienceData[activeProfile] || [];
    ...
    <div className="w-full max-w-[90rem] flex flex-col items-start">
      ...
      <div className="flex flex-col lg:flex-row w-full gap-[3rem] lg:gap-[5rem] min-h-[45rem]">
        ...
        {hasExperience && activeJob ? (
                ...
                >
                  <motion.div variants={childVariants}>
                    ...
                    <p className="text-font-third font-mono text-[1.4rem] mt-2 italic">
                      {t(`${activeJob.name}.from`)} – {t(`${activeJob.name}.to`)}
                    </p>
                  </motion.div>
                    ...
                    {Array.from({ length: activeJob.duties }).map((_, index) => (
                      ...
                      >
                        ...
                        <motion.p
                          ...
                          initial="rest"
                          whileHover="hover"
                          className="text-[1.5rem] text-font-third leading-[1.4] cursor-default"
                        >
          ...
          /* EMPTY STATE: Gdy dany profil nie ma jeszcze wpisów */
          <div className="w-full flex flex-col items-center justify-center py-[10rem] border border-dashed border-white/10 rounded-[20px] bg-white/5">
            <p className="text-font-third text-[1.8rem] text-center italic">
              {t("noExperienceYet") || "Currently building experience in this field. Stay tuned!"}
            </p>
          </div>
        )}
      </div>
